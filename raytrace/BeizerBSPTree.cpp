...
BoundingBox BeizerBSPTree::get_boundingbox(int t, int theta)
{
	...
	double rotate_min = (double)(theta / 16) * PI;
	double rotate_max = (double)((theta + 1) / 16) * PI;
...
BeizerBSPTree_Node** BeizerBSPTree::merge_child(int t_num, int theta_num, BeizerBSPTree_Node** Childs, bool merge_type)     //若merge_type为true，合并t_num,否则,合并theta_num
{
	...
	{
		for (int i = 0; i < (t_num / 2) * theta_num; i += 2)
		{
...
bool BeizerBSPTree::RayTreeIntersect(Ray input_ray, BeizerBSPTree_Node* tree_node, vector3<double>& point)
{
	if (tree_node->left_child == nullptr && tree_node->right_child == nullptr)           //leaf node,迭代求解
	{
